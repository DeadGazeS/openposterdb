...
0. **Logo outer margins inconsistent across tb/bt/rl/lr — FIXED 2026-08-03**:
   - **User requirement**: the source logo's three OUTER margins (facing the badge frame) must be EQUAL at 100%, and the INWARD side (facing the text) must have barely any margin (~1-2px), so the text box takes the remaining space and centres the text. This must hold uniformly for all four styles. Also: UI wording "value" → "Number" (BADGE_STYLE_LABELS — the badge value is always a number, e.g. "Logo top · Number bottom"), and README.md "logo/value arrangement" → "logo/number arrangement".
   - **Measured before the fix (icon label styles, 100%, square 48×48 icon, rounded + pill)**: lr/rl → L=6 T=5 R=6 B=5 (logo centred in its label section, inward side = 6px); tb/bt → L=6 R=6 T=5 B(inward)≈4. So (a) the three outer sides are never equal (5 vs 6 — vertical is capped at 5 by (58−48)/2), (b) the inward side is 4-6px, not "barely", (c) margins differ per style. Tests encoding the old contract: TestVerticalBadgeLogoMarginsMatchHorizontal, TestVerticalLogoInwardMarginMinimal, TestVerticalValueTextCentredLikeHorizontal.
   - **Fix implemented**: uniform rule — equal outer margin m = (badgeH − iconH)/2 on the three frame sides (5px rounded / 8px pill at 100%; constraint-bound by the 48px icon in the 58px (64px pill) badge), inward gap = badgeInwardGap (2px), logo pinned against the seam instead of centred in its section, tb/bt value text now CENTRED in its (fixed) value section (was hugging the seam); measured after fix (48×48 icon, 100%): lr/rl → outer 5/5/5, inward 2; tb/bt → outer 5/5/5, inward 2 (4 to the text box = 2 inward + 2 inter-section gap); pill → outer 8/8/8, inward 2.
   - **Implementation**: renderBadgeInner sizes the icon label section as baseIconW + iconPad + badgeInwardGap and pins the logo's INWARD edge to the seam (oversized logos fall back to centred clipping so they clip evenly); RenderVerticalBadge sets the side margin = the top/bottom margin so vLogoW = vertBadgeW − 2m; the logo clip no longer insets the seam side; rl-pill pillPad placement fixed so the cap padding always lands on the VALUE section's outer end (true lr/rl mirror symmetry). Follow-ups from review: (1) the tb/bt logo no longer resizes with badge_height — the width margin is fixed at the 100%-height reference so extra height becomes padding (guarded by TestVerticalLogoSizeIndependentOfBadgeHeight); (2) the horizontal pill cap padding is now FILLED (the value tile extends to the badge edge) so artwork never shows through the pill tip (pre-existing transparent strip; guarded by a pill-tip alpha probe in TestLogoUniformOuterMarginsAndInwardMinimal); (3) the horizontal icon section's outer margin is fixed by the 100%-height reference too, so the badge width never tracks badge_height (per-axis isolation, mirroring the vertical fix). Tests: TestVerticalBadgeLogoMarginsMatchHorizontal (comments updated); TestVerticalLogoInwardMarginMinimal → TestLogoUniformOuterMarginsAndInwardMinimal (4 styles × rounded/pill: outer equal ±1, inward ≤ 5, logs margins); TestVerticalValueTextCentredLikeHorizontal now asserts ink-centring in the value section; verticalSectionY gap corrected 4→badgeInwardGap; horizontalValueSection mirrors the new icon section formula. UI/README: BADGE_STYLE_LABELS + the SelectItems (RenderSettingsForm ×4, FreeApiKeyCard) "value"→"Number" (e.g. "Logo top · Number bottom"); README.md "logo/value arrangement" → "logo/number arrangement"; web specs updated (constants.spec, FreeApiKeyCard.spec/.display.spec). Verified: go build/vet/test all green; vue-tsc --noEmit clean; vitest still unrunnable in this sandbox (EACCES on node_modules/.vite-temp). Uncommitted (per Git Rules).

0. **Poster fit real blur + fit descriptions (2026-08-03)**:
   - **Blur was never blurred**: `fitPoster`'s blur case filled the bars with a sharp zoomed copy of the poster (comment: "we just do pad for now"). Now the fill runs through a real (dependency-free) blur: extracted the episode spoiler-blur's downscale-1/4/upscale trick into `blurImage(img, divisor)` in internal/image/generate.go, reused it for the episode path (divisor 4, byte-identical behavior) and for the poster blur-fill (divisor 4). For a 2:3 poster the blur stays visually identical to native/pad (no bars exist to fill — honest); for non-2:3 artwork the bars now show a genuinely smoothed copy.
   - **UI**: the poster fit select's label "Aspect ratio" → "Fit" (it's a fit mode, not an aspect ratio); added `POSTER_FIT_DESCRIPTIONS` in web/src/lib/constants.ts and a live description line under the select (data-testid `poster-fit-description`) explaining the currently selected option.
   - Tests: new generate_test.go (TestBlurImageSmooths checkerboard variance, TestBlurImageSmallUnchanged, TestPosterBlurFillIsBlurred vs pad/cover); TestPreviewFitModesMatchRealRender still green. go build/vet/test all green; vue-tsc --noEmit clean.
   - **Environment note**: web unit tests (vitest) and `vue-tsc --build` could not run in this sandbox — `web/node_modules` is owned by `nobody:nogroup` (read-only for the dev user), so vite/vitest cache writes (`node_modules/.vite-temp`, `.tmp/*.tsbuildinfo`) fail with EACCES. Per the rules the environment was not "repaired". Verified instead with `vue-tsc --noEmit -p tsconfig.app.json` (exit 0). Uncommitted (per Git Rules).

0. **Poster preview fit faithfulness (2026-08-03, fixed)**:
   - **Bug**: the settings preview centre-cropped the demo 2:3 poster to a 3:2 band (and composed fit=native over a dimmed backdrop) so the four fit modes looked artificially distinct — that cut the top/bottom ~¼ off the poster under the default fit=native, while the real image endpoint serves the full poster. Reported: "a lot of the image is cut off… doesn't happen when i fetch the poster normally."
   - **Fix**: `PosterPreviewArtwork` (internal/image/preview_artwork.go) is now a passthrough — the preview feeds the unchanged artwork into the same `fitPoster` logic as the real render. For a 2:3 poster all four fits render the full poster onto the 2:3 canvas (identical output — honest, since the real pipeline is identical for a 2:3 source); for non-2:3 artwork the fits still differ (native keeps the aspect, cover/pad/blur fill 2:3). The web preview box sizes from the rendered image's natural dimensions (aspect-ratio style), so the portrait box is stable without the old canvas hack.
   - **Removed**: the 3:2 centre-crop, the native dimmed-backdrop composition, and the dead helpers (encodePNG, isPosterAspect, posterAspectTolerance, posterPreviewBackdropKeep). Tests rewritten: TestPosterPreviewArtworkPassthrough, TestPosterPreviewArtworkDecodesJPEG (kept), TestPreviewFitModesMatchRealRender (2:3 source → all fits identical + native == real render; non-2:3 source → four distinct outputs).
   - Verified: go build/vet/test all green; live preview returns 580×870 JPEG for every fit; native preview pixels match the full sample gradient (top 41 / mid 34 / bottom 26 vs the 42→26 source). Uncommitted (per Git Rules).

0. **Badge text centring + tb/bt fixed box round (2026-08-03)**:
   - **Horizontal value-text centring (lr/rl)**: value text was centred on the glyph ADVANCE width (ignores side bearings) with a `badgeH/2 + ascentVal/2 - 2` baseline hack, leaving the ink ~1px off (x and y). Now centred by the exact INK bounding box (`textInkBBox` — offscreen render + pixel scan) on both axes, and clipped to the value section via `drawTextShadowedInRect` (SubImage) so oversized text at textScale>1 clips evenly instead of spilling into the label section. The 13a1be8 formula is algebraically identical to the old one — the offset was glyph optics, not a formula regression. New badge_centring_test.go: ink-centre within ≤1px for lr/rl at scale 1/2, even clipping.
   - **tb/bt fixed badge box**: RenderVerticalBadge previously multiplied section heights by textScale (`sectionH × textScale`), so the vertical badge BOX grew with text size — violating the fixed-box rule (horizontal badgeH/totalW never changes with textScale). Sections are now fixed by badgeScale only (`badgeHeight (+pillPaddingV)`); text scales inside and clips evenly. badge_vertical_test.go asserts identical box dims at textScale 1 vs 2 (rounded + pill).
   - **tb/bt logo outer margins**: the vertical logo was vertically centred in its section, so its top/bottom margin (5px) never matched the side margin (20px). Now positioned so the three outer margins are equal — tb: right=left=top; bt: right=left=bottom — with a corner-safe centred fallback for icons wider than the badge.
   - Verified: go build/vet/test all green (badge_centring_test.go, badge_vertical_test.go, text_inkbox_test.go). Uncommitted (per Git Rules).

0. **Full audit round (2026-08-03, all findings fixed)**:
   - **Serve path ratings limit**: `ServeImage` now takes an explicit `?ratings_limit` override (`ratingsLimitForKind` = valid override else per-kind `layout.Total()`); the stored legacy `RatingsLimit`/`*RatingsLimit` fields are never consulted (they can be 0 for per-key rows). Preview handlers apply `ValidateRatingsLimit` (invalid/absent override → layout total) so `>10` no longer flows through.
   - **Per-key settings PUT → partial update**: `HandleUpdateKeySettings`/`HandleUpdateSelfSettings` decode into a `keySettingsUpdate` wire struct (pointer fields for the four ratings limits + three badge directions the web omits), load the stored row (or defaults for fresh rows), overlay only provided values, then validate/normalize (lang, order/exclude, limits 0–10, layouts, enums via Parse*, alpha 0–100, insets 0–50) — mirroring the global path's validation. Omitted field ⇒ stored value preserved; explicit 0 honored.
   - **Schema cleanup**: the three dead `ADD COLUMN logo/backdrop/episode_badge_background` migrations became `DROP COLUMN` (with "no such column" ExpectedError) so deployed DBs clean up; the legacy poster_badge_background DROP stays.
   - **Gallery badge mirrors poster style**: icons.go resolves the poster badge style (Resolve + direction default) into `BadgeAppearance.Style` and routes tb/bt through `RenderVerticalBadge`, so Rating-Colours badges match the poster preview instead of always rendering horizontal.
   - **FreeApiKeyCard `badge_background` → `badge_alpha`**: the dead Background select (d/k/t/n) is replaced by a per-kind "Background opacity %" number input emitting `*_badge_alpha` (empty = server default, placeholder "default (80%)"); `auth-api.ts` FreeKeyDefaults swapped `*_badge_background: string` for `*_badge_alpha: number`; `badgeShape` (+ alpha) now reset per image-type switch.
   - **Edge-inset preview coercion**: the backdrop preview fetch passes `coerceInset(Number(...))` so a cleared/partial input previews the same clamped int the save path persists.
   - **Poster badge-style normalization**: `'d'`/`'h'`/empty → `'lr'` on load (`normalizePosterBadgeStyle` in ref init, applySettings, settingsSnapshot) so the 4-option poster select never renders blank on fresh install (server default is `'d'`).
   - **e2e suite rewritten to the current contract**: settings.spec (17→16), key-settings.spec (13→11), free-api-key.spec (20, kept) — explicit Save/Discard flow, layout-grid-driven counts, eye-toggle exclusion, no auto-save/ratings-limit inputs/position dropdowns; `setFreeApiKey` helper now sends a minimal `{ free_api_key_enabled }` PUT (the old echo-helper 400'd because GET layouts are strings but the PUT expects objects); new `resetGlobalSettings` helper for deterministic state.
   - Verified: go build/vet/test all green (incl. new serve/api_keys/icons tests); vue-tsc clean; vitest 28/28 files, 340 tests pass. Uncommitted (per Git Rules).

0. **Settings nav + ratings eye + form rework round (2026-08-03)**:
   - **Settings sections moved to the sidebar**: `/admin/settings` now redirects to `/admin/settings/general`; `settings/general` and `settings/image` are separate routes rendering SettingsView with a route-derived active section (`route.name === 'settings-image'`). The in-page "Sections" nav was removed; AppSidebar has a collapsible Settings group (reka-ui Collapsible + SidebarMenuSub) with General/Image sub-items, auto-expanding on settings routes. Page headline is now the section name (General/Image), not "Settings".
   - **Ratings eye toggle replaces the Exclude ratings section**: the checkbox grid per ALL_RATING_SOURCES is gone. RatingsOrderList gained an optional `toggleable` prop + `toggle-exclude` emit; each row shows a plain Eye/EyeOff button (data-testid `exclude-<key>-eye`) to the left of the move arrows — solid eye = shown, faded/crossed = excluded. RenderSettingsForm wires it to `editRatingsExclude`/`toggleExclude`. FreeApiKeyCard keeps its own checkbox grid (toggleable defaults off).
   - **Badge style selects: exactly 4 options** (lr/rl/tb/bt — no Auto/d) across poster/logo/backdrop/episode; BADGE_STYLE_LABELS in constants.ts maps lr/rl/tb/bt/d.
   - **Badge direction selects removed** (poster/backdrop/episode) — redundant with the 4 explicit styles. Script plumbing (edit*BadgeDirection refs, snapshot/save payload entries) removed; preview fetch calls pass the fixed `'d'` argument (fetch prop signatures and views unchanged). Backend BadgeDirection fields remain (omitted = keep stored value).
   - **Per-kind grid order**: badge style → label style → badge shape → (text/badge/logo size → background opacity → fit/extras); shape select moved up to third position in all four grids.
   - **Rating Source Badges gallery removed** — the gallery section (ALL_RATING_SOURCES badge samples with captions) is deleted; each Rating Colours row now shows source name text → badge preview(s) (top-aligned with the name, no caption under the badge) → colour controls. The `logo bg · text bg · border · text` per-row caption was removed (legend above covers it).
     - Follow-up: RT rows are one row per logo variant (rt_cf/rt_pos/rt_rot/rta_*), and each row shows ONLY the variant matching its row label (`badgeSamplesForRow` in RenderSettingsForm.vue maps the row key to the base source rt/rta, derives the variant from the row label, and returns a single-element array; non-RT rows return their single sample unchanged).
   - **LayoutEditor grid**: compacted to fixed `max-w-[300px]` and tightened (p-2 cards, text-xs labels, h-8 inputs, all testids preserved). Layout is **2 rows × 2 settings** (`grid grid-cols-2`): row 1 Top | Bottom, row 2 Left | Right (side order `['top', 'bottom', 'left', 'right']`).
   - **All four previews same size**: poster/logo/backdrop/episode preview containers all use `w-[220px]` (was 220/240/360/360), keeping the aspectRatio style binding.
   - **Discard changes now refreshes previews**: `revertEdits()` (discard + save-failure revert) previously re-applied settings while `syncing` blocked all preview watchers and never refreshed afterwards — previews stayed stale. It now mirrors the props.settings watcher: after `nextTick` clears `syncing`, it compares `editsSnapshot()` before/after and calls `updateAllPreviews()` when the values actually changed.
   - **Settings form tab persistence**: the form's outer Tabs previously used `default-value="image-settings"`, so a page refresh always reset to the first tab. Now `activeTab` is a ref bound via `:model-value` + `@update:model-value` and persisted to localStorage under `render-settings-active-tab` (updated on change, restored on load).
   - **tb/bt vertical geometry fixed** (badge.go RenderVerticalBadge): tb/bt were NOT mirror images (tb had zero top padding + doubled gap; bt had padded value + half gap) — now symmetric (`vertPadV` top, full `gap` between sections), matching the lr/rl horizontal symmetry. Render-branch selection in generate.go now uses resolved `appearance.Style.IsVertical()`; HandleLogo preview default corrected from lr to tb (matches the real logo default).
   - **vite-plugin-placeholders.ts**: type query `typeof import('sharp')['default']` failed (sharp uses `export =` typings) — now `typeof import('sharp')`; the empty-placeholder fallback when sharp can't be imported still resolves `virtual:placeholders` under the vitest module runner.
   - Verified: `go build/vet/test` all pass; vue-tsc clean; vitest 28/28 files, 331 tests pass. Uncommitted (per Git Rules — awaiting explicit commit/push request).
